    ...
    try:
        if request.method == "POST":
            try:
                ...
                print(error)
                return internal_error_response(error=str(error))
            if not (isinstance(user_name, str) or tools.user_username_checker(user_name)):
                status_code = 400
                response_msg = {
                    "code": 2,
            ...
            elif not (isinstance(password, str) or tools.user_password_checker(password)):
                status_code = 400
                response_msg = {
                    "code": 3,
            ...
            if "new_user_name" in request_data:
                ...
                if not old_user_name == new_user_name:
                    try:
                        ...
                        if not (isinstance(new_user_name, str) or tools.user_username_checker(new_user_name)):
                            status_code = 400
                            response_msg = {
                                "code": 3,
    ...
    if request.method == "POST":
        ...
        try:
            ...
            if not user:  # user name not existed yet.
                ...
                elif not (isinstance(new_password, str) or tools.user_password_checker(new_password)):
                    status_code = 400
                    response_msg = {
                        "code": 3,
                        "message": "INVALID_PASSWORD_FORMAT",
